...
**Endpoint base:** `https://declarador.io`
**Versión:** v1
...
Contacta al administrador de **declarador.io** para solicitar una API Key para tu sistema. Se te asignará una clave de 64 caracteres que identifica tu integración.
...
curl -H "Authorization: Bearer <api_key>" \
  https://declarador.io/api/v1/declaracion/AB12CD34/

# Por validation_hash
curl -H "Authorization: Bearer <api_key>" \
  https://declarador.io/api/v1/declaracion/ABCDEF0123456789/

# En inglés
curl -H "Authorization: Bearer <api_key>" \
  "https://declarador.io/api/v1/declaracion/AB12CD34/?lang=en"
```
...
| `author_orcid` | string | No | ORCID del autor (formato `0000-0000-0000-0000`) |
...
| `human_review.level` | integer | No | Nivel de revisión 0–5 (default: `0`) |
...
| `selected_checklist_ids` | array | No | IDs de preguntas diagnósticas respondidas |
...
| `5` | Validación experta | Revisión profunda por experto en la materia **(ORO)** |
...
```bash
curl -X POST https://declarador.io/api/v1/declaracion/ \
...
```json
{
  ...
  "declaration_url": "https://declarador.io/declaracion/K7M2P9XQ/",
...
```json
{
  ...
  "declaration_url": "https://declarador.io/declaracion/N3Q8R1TZ/",
...
const BASE_URL = 'https://declarador.io';
...
function crear_declaracion(array $datos): array {
    $api_key = 'tu_api_key_aqui';
    $url = 'https://declarador.io/api/v1/declaracion/';
...
*Documentación generada para declarador.io — API v1*
